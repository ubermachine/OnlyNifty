...
class TelegramNotifier:
    ...
    def format_signal_html(self, entry: Any) -> str:
        ...
        grade = html.escape(str(getattr(entry, "confluence_grade", "A Standard")))
        ...
        ts_ist = html.escape(str(getattr(entry, "timestamp_ist", "")))
        reason = html.escape(str(getattr(entry, "trigger_reason", "")))
        regime = html.escape(str(getattr(entry, "regime_summary", "CONFLUENCE")))
        is_0dte = getattr(entry, "is_0dte", False)

        otm_hedge_strike = strike + 100 if is_long else strike - 100

        html_text = f"""<b>{action_emoji} ONLYNIFTY INSTITUTIONAL SIGNAL ALERT</b>
━━━━━━━━━━━━━━━━━━━━
...
⏰ <i>{ts_ist} | Generated by OnlyNifty v5.2</i>"""
    ...
    def format_lifecycle_html(self, entry: Any, status_event: str, current_spot: float, current_prem: float) -> str:
        ...
        html_text = f"""<b>{emoji} ONLYNIFTY TRADE UPDATE: {title}</b>
━━━━━━━━━━━━━━━━━━━━
